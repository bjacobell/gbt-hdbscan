...
out_directory = '/datax/scratch/benjb/injections'       # Output directory.
...
file_path = '/datag/pipeline/AGBT18A_999_103/collate/spliced_blc0001020304050607_guppi_58306_31766_DDO210_0002.gpuspec.0000.h5'
...
def main():
    ...
    #driftlist = np.linspace(-4, 4, num=int(num_to_insert*num_frames))
    ...
    drifts = (np.random.uniform(-4, 4, 1) for _ in range(int(num_frames)))

    # Bandwidth of signal in units of fine channel widths
    # widths = ([2.0 for _ in range(num_to_insert)]
    #           for _ in range(int(num_frames)))
    #widthlist = np.linspace(2.0, 8.0, num=int(num_to_insert*chans/fchans))
    #widths = (widthlist[num_to_insert*N:num_to_insert*(N+1)]
    #         for N in range(int(num_to_insert*chans/fchans)))
    widths = (np.random.uniform(2.0, 8.0, 1) for _ in range(int(num_frames)))

    # Insert all signals at 1000 SNR
    # snrs = ([1000.0 for _ in range(num_to_insert)]
    #         for _ in range(int(num_frames)))
    #snrlist = np.linspace(80, 1000, num=int(num_to_insert*chans/fchans))
    #snrs = (snrlist[num_to_insert*N:num_to_insert*(N+1)]
    #         for N in range(int(num_to_insert*chans/fchans)))
    snrs = (np.random.uniform(1000, 10000, 1) for _ in range(int(num_frames)))
